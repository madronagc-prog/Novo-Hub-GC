import { servicesGCData } from './src/data/servicesGCData_backup.ts';
import fs from 'fs';
...
if (oldMap.has('decisoes')) oldMap.set('decisões', oldMap.get('decisoes'));
if (oldMap.has('rt-online')) oldMap.set('rt online', oldMap.get('rt-online'));
...
userList.forEach(group => {
  group.items.forEach(itemName => {
    ...
    if (!oldObj) {
      const match = Array.from(oldMap.values()).filter(Boolean).find(s => s.name.toLowerCase() === itemName.toLowerCase() || (s.fullName && s.fullName.toLowerCase() === itemName.toLowerCase()));
