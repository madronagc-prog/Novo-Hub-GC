...
import { defaultClippingData } from './clippingData';

export default function ClippingAbril2026() {
  ...
  const [clippingData, setClippingData] = useState<any>(defaultClippingData);
  const [editDataStr, setEditDataStr] = useState("");

  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (u) => {
      if (u) {
        setIsAdmin(true); // simplify or check role based on your logic
      } else {
        setIsAdmin(false);
      }
    });
  ...
  useEffect(() => {
    const docRef = doc(db, 'clippings', 'abril-2026');
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        setClippingData(docSnap.data().data);
      } else {
        setClippingData(defaultClippingData);
      }
    }, (error) => {
      console.error(error);
    });
  ...
  const handleSaveDoc = async () => {
    try {
      ...
      await setDoc(doc(db, 'clippings', 'abril-2026'), { data: parsed });
      setIsEditing(false);
    } catch (e) {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center">
            ...
            <div className="inline-flex items-center gap-2 bg-brand-grafite/80 px-4 py-2 rounded-full text-brand-cinza text-sm font-medium border border-slate-700">
              <Calendar size={16} />
              <span>1ª Edição — Abril 2026</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="bg-white border-b border-gray-200 sticky top-20 z-10 shadow-sm overflow-x-auto">
        <div className="max-w-4xl mx-auto px-4 min-w-max sm:min-w-0">
          <div className="flex items-center gap-6 whitespace-nowrap py-4 justify-start sm:justify-center text-sm font-medium">
            <a href="#jurisprudencia" className="text-brand-grafite hover:text-brand-grafite transition-colors">Jurisprudência</a>
            <a href="#legislacao" className="text-brand-grafite hover:text-brand-grafite transition-colors">Legislação</a>
            <a href="#doutrinas" className="text-brand-grafite hover:text-brand-grafite transition-colors">Doutrinas</a>
              ...
              {clippingData.jurisprudencia.map((item: any) => (
                <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8 hover:shadow-md transition-shadow">
                  ...
                  <div className="prose prose-sm text-brand-grafite mb-6 space-y-4">
                    {item.paragraphs?.map((p: string, i: number) => <p key={i}>{p}</p>)}
                  </div>
                  {item.link && (
            ...
            <div className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              {clippingData.doutrinas.map((item: any) => (
                <div key={item.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col hover:shadow-md transition-shadow">
                  ...
                  <p className="text-sm text-brand-grafite flex-1 mb-6">{item.content}</p>
                  {item.link && (
