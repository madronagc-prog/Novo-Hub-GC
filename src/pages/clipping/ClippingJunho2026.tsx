...
import { defaultClippingJunhoData } from './clippingJunhoData';

export default function ClippingJunho2026() {
  ...
  const [clippingData, setClippingData] = useState<any>(defaultClippingJunhoData);
  ...
  useEffect(() => {
    const docRef = doc(db, 'clippings', 'junho-2026');
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        setClippingData(docSnap.data().data);
      } else {
        setClippingData(defaultClippingJunhoData);
      }
    }, (error) => {
      console.error(error);
    });
  ...
  const handleSaveDoc = async () => {
    try {
      ...
      await setDoc(doc(db, 'clippings', 'junho-2026'), { data: parsed });
      setIsEditing(false);
    } catch (e) {
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center">
            ...
            <div className="inline-flex items-center gap-2 bg-brand-grafite/80 px-4 py-2 rounded-full text-brand-cinza text-sm font-medium border border-slate-700">
              <Calendar size={16} />
              <span>3ª Edição — Junho 2026</span>
            </div>
          </div>
        </div>
      </header>
